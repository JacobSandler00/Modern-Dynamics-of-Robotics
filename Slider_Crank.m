function [t,VAR,Output] = Slider_Crank
%===========================================================================
% File: Slider_Crank.m created Oct 07 2021 by MotionGenesis 6.1.
...
qb                              =  44.47751218592993;      % deg                 Initial Value
qc                              =  14.47751366450409;      % deg                 Initial Value
qa                              =  0.5235987755982988;     % rad                 Initial Value

tInitial                        =  0.0;                    % second              Initial Time
tFinal                          =  12;                     % seconds             Final Time
tStep                           =  0.2;                    % seconds             Integration Step
...
qbDt = qaDt*(La*cos(qa)+Lb*cos(qa-qb))/(Lb*cos(qa-qb));
qcDt = qaDt*(-1+(La*cos(qa)+Lb*cos(qa-qb))/(Lb*cos(qa-qb)));
...
%===========================================================================
x = La*cos(qa) + Lb*cos(qa-qb);
y = La*sin(qa) + Lb*sin(qa-qb);
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files Slider_Crank.i  (i=1,2)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(1) = fopen('Slider_Crank.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file Slider_Crank.1'); end
      fprintf(FileIdentifier(1), '%% FILE: Slider_Crank.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%       t              x\n' );
      fprintf(FileIdentifier(1), '%%   (seconds)         (m)\n\n' );
      FileIdentifier(2) = fopen('Slider_Crank.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file Slider_Crank.2'); end
      fprintf(FileIdentifier(2), '%% FILE: Slider_Crank.2\n%%\n' );
      fprintf(FileIdentifier(2), '%%       t              y\n' );
      fprintf(FileIdentifier(2), '%%   (seconds)         (m)\n\n' );
   end
...
%===========================================================================
figure;
data = load( 'Slider_Crank.1' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
legend( 'x (m)' );
xlabel('t (seconds)');   ylabel('x (m)');   % title('Some plot title');
clear data;

figure;
data = load( 'Slider_Crank.2' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
legend( 'y (m)' );
xlabel('t (seconds)');   ylabel('y (m)');   % title('Some plot title');
clear data;
end
...
%=====================================
end    % End of function Slider_Crank
%=====================================
