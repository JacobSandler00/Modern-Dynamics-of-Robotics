function [t,VAR,Output] = PogoStickHunt
%===========================================================================
% File: PogoStickHunt.m created Nov 02 2021 by MotionGenesis 6.1.
...
yaDDt=0; ybDDt=0; x=0; xDt=0; FHC=0;


%-------------------------------+--------------------------+-------------------+-----------------
% Quantity                      | Value                    | Units             | Description
%-------------------------------|--------------------------|-------------------|-----------------
bhc                             =  1e5;                    % UNITS               Constant
c                               =  2;                      % N*s/m               Constant
g                               =  9.81;                   % m/s^2               Constant
k                               =  1000;                   % N/m                 Constant
khc                             =  1e6;                    % UNITS               Constant
ln                              =  0.25;                   % m                   Constant
ma                              =  0.8;                    % kg                  Constant
mb                              =  0.2;                    % kg                  Constant
n                               =  1.5;                    % UNITS               Constant
...
FHC = khc*x^n + bhc*x^n*xDt;
ybDDt = -g - (k*(ln-ya)-FHC-c*yaDt)/mb;
yaDDt = (k*(ln-ya)-c*yaDt)/ma + (k*(ln-ya)-FHC-c*yaDt)/mb;
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files PogoStickHunt.i  (i=1,2)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(1) = fopen('PogoStickHunt.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file PogoStickHunt.1'); end
      fprintf(FileIdentifier(1), '%% FILE: PogoStickHunt.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%       t             ya\n' );
      fprintf(FileIdentifier(1), '%%     (sec)           (m)\n\n' );
      FileIdentifier(2) = fopen('PogoStickHunt.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file PogoStickHunt.2'); end
      fprintf(FileIdentifier(2), '%% FILE: PogoStickHunt.2\n%%\n' );
...
%======================================
end    % End of function PogoStickHunt
%======================================
