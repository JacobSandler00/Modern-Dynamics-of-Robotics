function [t,VAR,Output] = BB
%===========================================================================
% File: BB.m created Oct 19 2021 by MotionGenesis 6.1.
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      FileIdentifier(1) = fopen('BB.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file BB.1'); end
      fprintf(FileIdentifier(1), '%% FILE: BB.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%       t              y\n' );
...
legend( 'y (m)' );
xlabel('t (sec)');   ylabel('y (m)');   % title('Some plot title');
clear data;
end
